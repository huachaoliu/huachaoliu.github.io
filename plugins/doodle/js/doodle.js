(function (factory) {
    ...
    if (typeof exports === 'object') {
        ...
        factory(window.jQuery, window.fabric, hasModule);
    }
} (function ($, fabric, hasModule) {
    'use strict';
    var markIcons = {};
    ...
    var textTop = null;
    var textLeft = null;
    var canvasMenuBar = null;
    var markupCanvasAnchor = null;
    var writeTextHTMl = null;
    var transparentCircle = null;
    var transparentRect = null;
    var transparentArrow = null;
    var _target = null;
    var _type = '';
    var _css;
    var isFirefox = /Firefox/i.test(navigator.userAgent);
    var activeColor = '#15bbec';
    var outColor = '#555';
    var Doodle = function (target, options) {
        var dom = document.createElement('div');
        dom.id = 'doodleContainer';
        this.dom = dom;
        this.outControl = false; //从外部唤醒toolbar
        var defaults = {
            showMarkToggle: true,
            type: 'model',
            width: '100%',
        ...
        // toolbarh = params.type === 'pdf' ? 34 : 0;　//pdf有个title的高度.模型则没有。
        _type = params.type;
        //自定义按钮的背景
        ...
        canvasMenuBar.className = 'markupToolbar';
        markupCanvasAnchor.id = 'markupCanvasAnchor';
        target.appendChild(this.dom);
        _style.innerHTML = _css;
        document.head.appendChild(_style);
        this.doodleInit(params);
    };
    Doodle.prototype.doodleInit = function (config) {
        var self = this;
        canvasMenuBar.innerHTML = menuHtml;
        markupCanvasAnchor.innerHTML = '<canvas id="canvas" height="' + config.height + '" width="' + config.width + '"></canvas>';
        this.dom.appendChild(canvasMenuBar);
        this.dom.appendChild(markupCanvasAnchor);
        canvas = this.__canvas = new fabric.Canvas('canvas', { selection: false, isDrawingMode: false });
        fabric.Object.prototype.transparentCorners = false;
        fabric.Object.prototype.originX = fabric.Object.prototype.originY = 'center';
        // canvas.lowerCanvasEl.left = config.position.left + 'px';
        // canvas.lowerCanvasEl.top = config.position.top + 'px';
        // canvas.freeDrawingBrush.color = fill;
        // canvas.freeDrawingBrush.width = 3;
        this.toolbarEvents(config.showMarkToggle, config.language);
        ...
        canvas.on('mouse:down', function (options) {
            ...
            if (drawType != "") {
                switch (drawType) {
                    case "drawCircle":
                        self.drawCircle(options);
                        break;
                    case "drawText":
                        self.drawText(options);
                        break;
                    case "drawRectangle":
                        self.drawRectangle(options);
                        break;
                    case "drawEraser":
                        self.generateEraser(options);
                        break;
                    case "drawFreeLine":
                        self.drawFreeLine(options);
                        break;
                    default:
            ...
            if (drawType != "") {
                switch (drawType) {
                    ...
                    case "drawCircle":
                        ...
                        break;
                    case "drawEraser":
                        self.drawEraser(options);
                        break;
                    case "drawRectangle":
                        self.drawRectangle(options);
                        break;
                    case "drawArrow":
                        self.drawArrow(options);
                        break;
                    default:
                        // console.log("not action");
                        break;
        ...
        document.onkeydown = function (e) {
            ...
            var writeTxt = getId('writeText').value;
    ...
    Doodle.prototype.toolbarEvents = function (flag, lang) {
        text = '请输入文字';
        ...
        var markupToggle = getId('markupToggle');
        var drawMover = getId('drawMover');
        var drawRectangle = getId('drawRectangle');
        var drawCircle = getId('drawCircle');
        var drawFreeLine = getId('drawFreeLine');
        var drawText = getId('drawText');
        var drawEraser = getId('drawEraser');
        var drawingColor = getId('drawingColor');
        var drawArrow = getId('drawArrow');
        var writeText = getId('writeText');
        ...
        if (flag) {
            markupToggle.onclick = function () {
                btnFlag = !btnFlag;
                changeBtnState(this, btnFlag);
            };
        } else {
            markupToggle.style.display = 'none';
        }
        drawArray.map(function (drawBtn, i) {
            drawBtn.onclick = function () {
                if (this.id == 'drawingColor') {
                    return false;
                }
                if (drawType === this.id) {
                    drawType = '';
                    selectedBtnState(this, false);
                    if (this.id == "drawEraser") {
                    ...
                    selectedBtnState(this, true);
                    lockOrUnlockAll(canvas, false);
                    if (this.id === 'drawMover') {
                        lockOrUnlockAll(canvas, true);
                    }
                }
                if (this.id === 'drawFreeLine') {
    ...
    Doodle.prototype.setBtnToolState = function (flag) {
        drawArray.map(function (drawBtn) {
            if (flag && drawArray) {
    ...
    Doodle.prototype.drawFreeLine = function () {
    ...
    Doodle.prototype.drawRectangle = function (options) {
        if (transparentRect) {
    ...
    Doodle.prototype.drawCircle = function (options) {
    ...
    Doodle.prototype.drawArrow = function (options) {
        if (transparentArrow) {
    ...
    Doodle.prototype.drawText = function (options) {
    ...
    Doodle.prototype.drawEraser = function (options) {
        var x = options.e.pageX - offsetX;
        var y = options.e.pageY - toolbarh;
        if (options.e.changedTouches) {
            x = options.e.changedTouches[0].pageX;
    ...
    Doodle.prototype.generateEraser = function (options) {
        var x = options.e.pageX - offsetX;
        var y = options.e.pageY - toolbarh;
        if (options.e.changedTouches) {
            x = options.e.changedTouches[0].pageX;
    ...
    Doodle.prototype.canvasLoadFromJSON = function (serialized) {
        canvas.clear();
    ...
    function changeBtnState(self, btnFlag) {
        ...
        if (btnFlag) {
            ...
            selectedBtnState(drawFreeLine, true);
        ...
        for (var i = 0; i < drawArray.length; i++) {
            if (btnFlag) {
                drawArray[i].style.display = "block";
            } else {
                drawArray[i].style.display = "none";
            }
        }
    }
    ...
    //get target moving left and top
    ...
    function selectedBtnState(self, flag) {
        drawArray.map(function (drawItem, i) {
            if (flag && self === drawItem) {
                drawItem.style.backgroundColor = activeColor;
            } else {
                if (drawItem.className !== drawingColor.className) {+
                    drawItem.style.backgroundColor = outColor;
                }
            }
        });
    }
    function lockOrUnlockAll(canvas, value) {
