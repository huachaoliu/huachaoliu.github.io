...
var facotry = function facotry(className) {
  var type = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'div';

  var dom = document.createElement(type);
  if (className.indexOf(' ') > -1) {
    className.split(' ').map(function (cls) {
      return dom.classList.add(cls);
    });
  } else {
...
function addDom() {
  ...
  for (var _len = arguments.length, domList = Array(_len), _key = 0; _key < _len; _key++) {
    domList[_key] = arguments[_key];
  }

  domList.map(function (dom) {
    if (dom) {
...
var suffix = ['width', 'height', 'left', 'top'];
...
var getMaxXYNumber = function getMaxXYNumber(size) {
  var winw = window.innerWidth;
  var winh = window.innerHeight;
  size += 4; //border
  var x = Math.floor(winw / size);
  var y = Math.floor(winh / size);
  return { x: x, y: y, winw: winw, winh: winh };
};

//通过box的大小，来换算最大的画布。
var getPanelMaxSize = function getPanelMaxSize(size) {
  var _getMaxXYNumber = getMaxXYNumber(size),
      x = _getMaxXYNumber.x,
      y = _getMaxXYNumber.y,
      winw = _getMaxXYNumber.winw,
      winh = _getMaxXYNumber.winh;
...
var checkSize = function checkSize(max, win, size) {
  if (max >= win) {
...
module.exports = {
  ...
  addDom: addDom,
  setStyle: setStyle,
  getPanelMaxSize: getPanelMaxSize,
  checkSize: checkSize,
  makeScope: makeScope,
  getMaxXYNumber: getMaxXYNumber,
  addClass: addClass,
...
//先生成配置面板 
var configBoard = new _configuration2.default('.game_config');
//开始构建游戏
...
var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }(); //在开始游戏之前或游戏结束之后，对游戏的设置。
...
module.exports = function () {
  function Configuration(container) {
    ...
    this.snakeSizeInp.onchange = function (e) {
      _this.size = parseInt(e.target.value);
    };
    var degres = document.getElementsByName('degree');

    var _loop = function _loop(i) {
      degres[i].onchange = function (e) {
  ...
  _createClass(Configuration, [{
    ...
    value: function start() {
      ...
      var size = this.size > 50 ? 50 : this.size < 15 ? 15 : this.size;
      var speed = 200 - this.degres * 50;
      (0, _utils.addClass)(this._wrapper, 'hide');
      this.snake = new _snake2.default(document.querySelector('.container'), {
        configuration: this,
        size: size || 15,
        speed: speed || 200,
        length: 5
      ...
      document.addEventListener('keydown', function (e) {
        //开始游戏
        if (e.keyCode === 13 && !_this2.snake.died) {
          _this2.snake.start();
        }
        //暂停游戏
        if (e.keyCode === 32) {
    ...
    value: function gameOver(score) {
      ...
      var go = document.querySelector('.game_over');
      var _score = document.querySelector('.score');
      var rebuild = document.querySelector('.rebuild');
      var newGamw = document.querySelector('.new_game');
      _score.textContent = score;
      (0, _utils.removeClass)(go, 'hide');

      rebuild.onclick = function () {
        _this3.snake.rebuild('rebuild');
        (0, _utils.addClass)(go, 'hide');
      };

      newGamw.onclick = function () {
        _this3.snakeSizeInp.value = '';
        _this3.snake.rebuild('reset');
        (0, _utils.addClass)(go, 'hide');
        (0, _utils.removeClass)(_this3._wrapper, 'hide');
      };
    }
  }]);
...
module.exports = function () {
  function Snake(panel, options) {
    _classCallCheck(this, Snake);

    this.panel = panel;
    this.configs = _lodash2.default.assign({
      speed: 200,
      size: 10,
      length: 3,
      color: constants.DEFAULT_COLOR
    }, options);
    this.scope = (0, _utils.makeScope)(this.configs.length);
    this.score = 0;
    this.snake = [];
    this.moved = false;
  }
  ...
  _createClass(Snake, [{
    ...
    value: function build() {
      ...
      //初始化食物
      this.food = (0, _utils.facotry)('snake food');
      //随机生产坐标
      ...
      var _randomPosition = this.randomPosition(x, y),
    ...
    key: 'randomPosition',
    value: function randomPosition(x, y) {
      var refer = x > y ? y : x;
      var fx = Math.floor(Math.random() * refer);
      var fy = Math.floor(Math.random() * refer);
      var flag = false;
      ...
      if (flag) {
        var rand = (0, _utils.getMaxXYNumber)(this.configs.size);
        x = rand.x;
        y = rand.y;
      } else {
        this.position = { fx: fx, fy: fy };
        return { fx: fx, fy: fy };
      }
      //如果新的坐标在蛇的身体中，就重新获取
    ...
    key: 'updateFoodPlace',
    value: function updateFoodPlace() {
      var _randomPosition2 = this.randomPosition(this.x, this.y),
          fx = _randomPosition2.fx,
          fy = _randomPosition2.fy;
    ...
    value: function start() {
      ...
      this.timer = setInterval(function () {
        _this.cacheScope = _lodash2.default.clone(_this.scope);
        _this.updateSnake();
      }, this.configs.speed);
    }
  }, {
      ...
      for (var i = 0; i < length; i++) {
        ...
        if (this.checkLife(nextPoint, this.cacheScope)) {
          ...
          if (this.checkNextPointOrFood(nextPoint)) {
            // this.moved = false;
            var newSnake = (0, _utils.facotry)('snake');
    ...
    key: 'checkLife',
    value: function checkLife(point, scope) {
      var checked = false;
    ...
    value: function rebuild(type) {
      ...
      this.timer = null;
      this.scope = (0, _utils.makeScope)(this.configs.length);
      this.panel.childNodes.forEach(function (element) {
        _this3.panel.removeChild(element);
      });
      type === 'rebuild' && this.build();
    }
  }, {
    key: 'checkNextPointOrFood',
    value: function checkNextPointOrFood(point) {
      var checked = false;
...
var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }(); //构建面板
...
module.exports = function () {
  ...
  _createClass(Grid, [{
    key: 'generator',
    value: function generator(snake, food) {
      var _this = this;

      this._grid = (0, _utils.facotry)('grid');

      var _getPanelMaxSize = (0, _utils.getPanelMaxSize)(this._size),
          width = _getPanelMaxSize.width,
          height = _getPanelMaxSize.height;

      var size = width > height ? height : width;
      _utils.setStyle.call(this._grid, { width: size, height: size });
      this._grid.appendChild(food);
      snake.map(function (box) {
        return _this._grid.appendChild(box);
      });
      return this._grid;
    }
  }, {
    key: 'addEle',
    value: function addEle(snake) {
